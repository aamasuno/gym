...
if len(lista)==0:
    ...
    st.header(':green[Quitar el fondo]')
    st.info('Si deseas quitar el fondo a las fotografías, marca la casilla que se muestra a continuación. Esta opción aumenta\
            los tiempos de carga de las fotografías.')
    ...
    finiper = st.date_input(label="Escoge la fecha inicial del período de referencia (formato AAAA/MM/DD)",
    ...
    if len(fechas)==1:
        ...
        for pos in postr:
            ...
            if foto is not None:
                ...
                col1.subheader(':green['+fechas_fmt[0]+']')
                if sinfondo:
        ...
        for pos in postr:
            st.header(':green[Vista '+posicion[postr.index(pos)]+']')
            ...
            if foto2 is not None:
                ...
                with open(nfoto, "wb+") as f:
                    for chunk in foto2.iter_chunks(4096):
                          f.write(chunk)
                foto2.close()
                col2.subheader(':green['+ffin+']')
                if sinfondo:
                    col2.image(remove(Image.open(nfoto)))
                else:
                    col2.image(nfoto)
            else:
                col2.write(':green['+ffin+']')
